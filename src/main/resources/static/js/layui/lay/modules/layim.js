...
;layui.define(["layer", "laytpl", "upload"], function(i) {
    var version = "3.9.1"
        ...
        this.v = version,
            e("body").on("click", "*[layim-event]", function(i) {
                ...
                events[t] ? events[t].call(this, a, i) : ""
            })
    };
    //基础配置
    LAYIM.prototype.config = function(options) {
        ...
        if (layui.each(Array(5), function(i) {
            ...
            layui.each(options.skin, function(i, e) {
                a.unshift(e)
            }),
            options.skin = a,
            options = e.extend({
        ...
        ,
        //监听事件
        LAYIM.prototype.on = function(events, callback) {//events, callback i, a
            return "function" == typeof callback && (call[events] ? call[events].push(callback) : call[events] = [callback]),
                this
        }
        ,
        //获取所有缓存数据
        ...
        ,
        //打开一个自定义的会话界面
        LAYIM.prototype.chat = function(i) {
        ...
        ,
        LAYIM.prototype.setChatStatus = function(i) {
            var a = thisChat();
            if (a) {
        ...
        ,
        //接受消息
        LAYIM.prototype.getMessage = function(i) {
            return getMessage(i),
                this
        }
        ,
        LAYIM.prototype.notice = function(i) {
            return notice(i),
                this
        }
        ,
        //添加好友/群组
        LAYIM.prototype.add = function(i) {
            return add(i),
                this
        }
        ,
        LAYIM.prototype.setFriendGroup = function(i) {
            return add(i, "setGroup"),
                this
        }
        ,
        LAYIM.prototype.msgbox = function(i) {
            return V(i),
                this
        }
        ,
        LAYIM.prototype.addList = function(i) {
            return P(i),
                this
        }
        ,
        LAYIM.prototype.removeList = function(i) {
            return removeList(i),
                this
        }
        ,
        LAYIM.prototype.setFriendStatus = function(i, a) {
            var t = e(".layim-friend" + i);
            t["online" === a ? "removeClass" : "addClass"]("layim-list-gray")
        }
        ,
        LAYIM.prototype.content = function(i) {
            return layui.data.content(i)
        }
    ;
    var listTpl = function(i) {
        var a = {
            friend: "该分组下暂无好友",
            group: "暂无群组",
            history: "暂无历史会话"
        ...
        , elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>', '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}", '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', "</li>"].join("")
    ...
    layui.data.date = function(i) {
        ...
        layui.data.content = function(i) {
            ...
            return i = (i || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/'/g, "&#39;").replace(/"/g, "&quot;").replace(/@(\S+)(\s+?|$)/g, '@<a href="javascript:;">$1</a>$2').replace(/face\[([^\s\[\]]+?)\]/g, function(i) {
                ...
                return '<img alt="' + a + '" title="' + a + '" src="' + faces[a] + '">'
    ...
    ;
    var layimMain, b, w, k, C, S = function(i, a, n) {
        return i = i || {},
            e.ajax({
                url: i.url,
                type: i.type || "get",
        ...
        return mine = init.mine || {},
            ...
            create = function(a) {
                ...
                cache = e.extend(cache, obj),
                    popim(laytpl(y).render(obj)),
                (l.close || data.min) && T(),
                    layui.each(call.ready, function(i, a) {
                        a && a(obj)
                    })
            }
            ,
            cache = e.extend(cache, obj),
            data.brief ? layui.each(call.ready, function(index, item) {
                item && item(obj)
            }) : void (init.url ? S(init, create, "INIT") : create(init))
    }, popim = function(i) { //显示好友列表面板
        return layer.open({
            type: 1,
            area: ["260px", "520px"],
            ...
            success: function(elem) {
                layimMain = elem,
                    R(elem),
                cache.base.right && elem.css("margin-left", "-" + cache.base.right),
                b && layer.close(b.attr("times"));
                var a = []
                    , n = elem.find(".layim-list-history");
            ...
            cancel: function(i) {
                T();
                var a = layui.data("layim")[cache.mine.id] || {};
                return a.close = !0,
                    layui.data("layim", {
                        key: cache.mine.id,
                        value: a
        ...
        layimMain.on("contextmenu", function(i) {
        ...
        layimMain.find(".layim-list-history").on("contextmenu", "li", function(a) {
            var n = e(this)
            ...
            n.hasClass("layim-null") || (layer.tips(l, this, {
                tips: 1,
                time: 0,
                anim: 5,
                fixed: !0,
        ...
        return b && layer.close(b.attr("times")),
        layimMain && layimMain.hide(),
            ...
            layer.open({
                ...
                offset: "rb",
                resize: !1,
                content: '<img src="' + (cache.mine.avatar || layui.cache.dir + "css/pc/layim/skin/logo.jpg") + '"><span>' + (i || cache.base.title || "我的LayIM") + "</span>",
                move: "#layui-layim-close img",
                success: function(i, a) {
                    b = i,
                    cache.base.right && i.css("margin-left", "-" + cache.base.right),
                        i.on("click", function() {
                            layer.close(a),
                                layimMain.show();
                            var i = layui.data("layim")[cache.mine.id] || {};
                            delete i.close,
                                layui.data("layim", {
                                    key: cache.mine.id,
                                    value: i
        ...
        if (a[0]) {
            ...
            k && layer.close(k.attr("times")),
            1 !== s.find("li").length || o[0] || (d || w.css("width", 800),
                s.css({
                    height: w.height()
                }).show(),
                c.css("margin-left", "200px")),
            o[0] || (s.append(laytpl(v).render(l)),
                c.append(laytpl(f).render(l)),
                I(i),
                E()),
                O(s.find(".layim-chatlist-" + i.type + i.id)),
            o[0] || viewChatlog(),
        ...
        var u = C = layer.open({
            ...
            content: laytpl('<ul class="layui-unselect layim-chat-list">' + v + '</ul><div class="layim-chat-box">' + f + "</div>").render(l),
            success: function(a) {
                w = a,
                    a.css({
                        "min-width": "500px",
                ...
                "function" == typeof i.success && i.success(a),
                    ...
                    viewChatlog(),
                    q(),
                    layui.each(call.chatChange, function(i, a) {
                        a && a(thisChat())
                    }),
                    a.on("dblclick", ".layui-layim-photos", function() {
                        var i = this.src;
                        layer.close(openChat.photosIndex),
                            t.photos({
                                photos: {
                                    data: [{
                                        alt: "大图模式",
        ...
        return i = i || {},
            layer.close(add.index),
            add.index = layer.open({
                type: 1,
                area: "430px",
                title: {
                    friend: "添加好友",
        ...
        i.addClass(THIS).siblings().removeClass(THIS),
            d.addClass(SHOW).siblings(l).removeClass(SHOW),
        ...
        var i = thisChat()
            , a = cache.message[i.data.type + i.data.id];
        a && delete cache.message[i.data.type + i.data.id]
    };
    //聊天窗口的切换监听
    var thisChat = LAYIM.prototype.thisChat = function() { //获取当前聊天面板
        if (w) {
            var i = e(".layim-chat-list ." + THIS).index();
            var cont = w.find(".layim-chat").eq(i);
    ...
    var setHistory = function(data) {//记录历史会话
        var local = layui.data("layim")[cache.mine.id] || {}
            , obj = {}
            , history = local.history || {}
            , is = history[data.type + data.id];
        if (layimMain) {
            var historyElem = layimMain.find(".layim-list-history");
            if (data.historyTime = (new Date).getTime(),
                ...
                var msgItem = laytpl(listTpl({
                    type: "history",
                    item: "d.data"
                })).render({
                    data: obj
                });
                historyElem.prepend(msgItem);
        ...
        var e = thatChat.elem.find(".layim-chat-main ul");
        var l = cache.base.maxLength || 3e3;
        data.content = thatChat.textarea.val()
        if ("" !== data.content.replace(/\s/g, "")) {
            if (data.content.length > l){
                return t.msg("内容最长不能超过" + l + "个字符");
            }
            e.append(laytpl(elemChatMain).render(data));
            var param = {
                mine: data,
                to: thatChat.data
            }
            ...
            pushChatlog(message);
            layui.each(call.sendMessage, function(i, a) {
                a && a(param)
            })
        }
        chatListMore();
        ...
        if (!(device.ie && device.ie < 9)) {
            ...
            i.src = layui.cache.dir + "css/modules/layim/voice/" + cache.base.voice,
                i.play()
        }
    }, G = {}, getMessage = function(i) {
        i = i || {};
        var a = e(".layim-chatlist-" + i.type + i.id)
            , t = {}
            , l = a.index();
        if (i.timestamp = i.timestamp || (new Date).getTime(),
        i.fromid == cache.mine.id && (i.mine = !0),
        i.system || pushChatlog(i),
            G = JSON.parse(JSON.stringify(i)),
        cache.base.voice && voice(),
        !w && i.content || l === -1) {
            if (cache.message[i.type + i.id])
                cache.message[i.type + i.id].push(i);
            else if (cache.message[i.type + i.id] = [i],
            "friend" === i.type) {
                var s;
                layui.each(cache.friend, function(a, e) {
                ...
                var o;
                layui.each(cache.group, function(a, e) {
                    if (e.id == i.id)
                        return e.type = "group",
                            e.name = e.groupname,
                            cache.chat.push(e),
                ...
                i.name = i.name || i.username || i.groupname,
                    cache.chat.push(i);
            if ("group" === i.type && layui.each(cache.group, function(a, e) {
                if (e.id == i.id)
                    return t.avatar = e.avatar,
                        !0
            }),
                !i.system)
                return cache.base.notice && notice({
        ...
        if (w) {
            ...
            i.system ? l !== -1 && c.append('<li class="layim-chat-system"><span>' + i.content + "</span></li>") : "" !== i.content.replace(/\s/g, "") && c.append(laytpl(elemChatMain).render(i)),
                chatListMore()
        }
    }, Y = "layui-anim-loop layer-anim-05", V = function(i) {
        var a = layimMain.find(".layim-tool-msgbox");
    ...
    var pushChatlog = function(message) {
        ...
        var chatlog = local.chatlog[message.type + message.id];
        if (chatlog) {
            var t;
            layui.each(chatlog, function(a, e) {
                e.timestamp === message.timestamp && e.type === message.type && e.id === message.id && e.content === message.content && (t = !0)
            }),
            t || message.fromid == cache.mine.id || chatlog.push(message),
            chatlog.length > MAX_ITEM && chatlog.shift()
        } else
    ...
    }, viewChatlog = function() {
        var local = layui.data("layim")[cache.mine.id] || {};
        var thatChat = thisChat();
        var chatlog = local.chatlog || {};
        // var chatlog = getHistory(cache.mine.id) || {};
        var ul = thatChat.elem.find(".layim-chat-main ul");
        layui.each(chatlog[thatChat.data.type + thatChat.data.id], function(i, a) {
            ul.append(laytpl(elemChatMain).render(a))
        });
        chatListMore()
        ...
        var a, e = {}, l = layimMain.find(".layim-list-" + i.type);
        if (j[i.type])
            if ("friend" === i.type)
                layui.each(cache.friend, function(n, l) {
                    if (i.groupid == l.id)
                        return layui.each(cache.friend[n].list, function(e, t) {
                            if (t.id == i.id)
                                return a = !0
                        }),
                            a ? t.msg("好友 [" + (i.username || "") + "] 已经存在列表中", {
                                anim: 6
                            }) : (cache.friend[n].list = cache.friend[n].list || [],
                                e[cache.friend[n].list.length] = i,
                                i.groupIndex = n,
                                cache.friend[n].list.push(i),
                                !0)
                });
            else if ("group" === i.type) {
                if (layui.each(cache.group, function(e, t) {
                    if (t.id == i.id)
                        return a = !0
                }),
                    a)
                ...
                e[cache.group.length] = i,
                    cache.group.push(i)
            }
        if (!a) {
            var s = laytpl(listTpl({
                type: i.type,
                item: "d.data",
            ...
            if ("friend" === i.type) {
                ...
                o.find(".layui-layim-list").append(s),
                    o.find(".layim-count").html(cache.friend[i.groupIndex].list.length),
        ...
        var a = layimMain.find(".layim-list-" + i.type);
        j[i.type] && ("friend" === i.type ? layui.each(cache.friend, function(e, t) {
            layui.each(t.list, function(t, n) {
                if (i.id == n.id) {
                    ...
                    return l.find(".layui-layim-list>li").eq(t).remove(),
                        cache.friend[e].list.splice(t, 1),
                        l.find(".layim-count").html(cache.friend[e].list.length),
                    0 === cache.friend[e].list.length && l.find(".layui-layim-list").html('<li class="layim-null">该分组下已无好友了</li>'),
                        !0
                }
            })
        }) : "group" === i.type && layui.each(cache.group, function(e, t) {
            if (i.id == t.id)
                return a.find(">li").eq(e).remove(),
                    cache.group.splice(e, 1),
                0 === cache.group.length && a.html('<li class="layim-null">暂无群组</li>'),
                    !0
        }))
    }, chatListMore = function() {
        var i = thisChat()
    ...
    var hotkeySend = function() {
        ...
        textarea.off("keydown").on("keydown", function(e) {
                var tmp = layui.data("layim")[cache.mine.id] || {}
                    , keyCode = e.keyCode;
                if ("Ctrl+Enter" === tmp.sendHotKey)
                    return void (e.ctrlKey && 13 === keyCode && sendMessage());
                if (13 === keyCode) {
                    if (e.ctrlKey)
    ...
    var faces = function() {
        ...
        layui.each(i, function(i, e) {
            arr[e] = layui.cache.dir + "images/face/" + i + ".gif"
        })
        return  arr
    }(), stope = layui.stope;
    //在焦点处插入内容
    var focusInsert = function(i, a) {
        var e, t = i.value;
        i.focus(),
    ...
    var anim = "layui-anim-upbit", events = {
        status: function(i, a) {
            var t = function() {
                i.next().hide().removeClass(anim)
            }
                , n = i.attr("lay-type");
            if ("show" === n)
                stope(a),
                    i.next().show().addClass(anim),
        ...
        sign: function() {
            var i = layimMain.find(".layui-layim-remark");
        ...
        tab: function(i) {
            var a, e = ".layim-tab-content", t = layimMain.find(".layui-layim-tab>li");
            "number" == typeof i ? (a = i,
                i = t.eq(a)) : a = i.index(),
                a > 2 ? t.removeClass(THIS) : (events.tab.index = a,
                    i.addClass(THIS).siblings().removeClass(THIS)),
                layimMain.find(e).eq(a).addClass(SHOW).siblings(e).removeClass(SHOW)
        },
        spread: function(i) {
            var a = i.attr("lay-type")
                , e = "true" === a ? "false" : "true"
                , t = layui.data("layim")[cache.mine.id] || {};
            i.next()["true" === a ? "removeClass" : "addClass"](SHOW),
        ...
        search: function(i) {
            var a = layimMain.find(".layui-layim-search")
                , e = layimMain.find("#layui-layim-search")
                , t = a.find("input")
                , n = function(i) {
                var a = t.val().replace(/\s/);
                if ("" === a){
                    events.tab(0 | events.tab.index);
                } else {
                    for (var n = [], l = cache.friend || [], s = cache.group || [], o = "", d = 0; d < l.length; d++)
            ...
            !cache.base.isfriend && cache.base.isgroup ? events.tab.index = 1 : cache.base.isfriend || cache.base.isgroup || (events.tab.index = 2),
                a.show(),
                t.focus(),
        ...
        msgbox: function() {
            var i = layimMain.find(".layim-tool-msgbox");
            return layer.close(events.msgbox.index),
                i.find("span").removeClass(Y).html(""),
                events.msgbox.index = layer.open({
                    type: 2,
                    title: "消息盒子",
                    shade: !1,
                    maxmin: !0,
        ...
        find: function() {
            return layer.close(events.find.index),
                events.find.index = layer.open({
                    type: 2,
                    title: "查找",
                    shade: !1,
                    maxmin: !0,
        ...
        skin: function() {
            layer.open({
                ...
                zIndex: 66666666,
                resize: !1,
                content: laytpl(m).render({
                    skin: cache.base.skin
                })
            })
        },
        about: function() {
            layer.alert("版本： " + version + '<br>版权所有：<a href="#" target="_blank">layim.layui.com</a>', {
                title: "关于 LayIM",
                shade: !1
            })
        },
        setSkin: function(i) {
            var a = i.attr("src")
                , e = layui.data("layim")[cache.mine.id] || {};
            e.skin = a,
            a || delete e.skin,
                layui.data("layim", {
                    key: cache.mine.id,
            ...
            layui.each(call.setSkin, function(i, e) {
        ...
        chat: function(i) {
            var a = layui.data("layim")[cache.mine.id] || {}
                ...
                , l = {};
            "friend" === e ? l = cache[e][t].list[n] : "group" === e ? l = cache[e][n] : "history" === e && (l = (a.history || {})[t] || {}),
        ...
        groupMembers: function(i, a) {
            ...
            i.data("down") ? l() : (n.html("&#xe619;"),
                i.data("down", !0),
                events.groupMembers.index = layer.tips('<ul class="layim-members-list"></ul>', i, {
                    tips: 3,
                    time: 0,
                    anim: 5,
                    fixed: !0,
        ...
        setSend: function(i, a) {
            var t = events.setSend.box = i.siblings(".layim-menu-box")
                , n = i.attr("lay-type");
            if ("show" === n)
                stope(a),
                    t.show().addClass(anim),
                    e(document).off("click", events.setSendHide).on("click", events.setSendHide);
            else {
                i.addClass(THIS).siblings().removeClass(THIS);
                var l = layui.data("layim")[cache.mine.id] || {};
                l.sendHotKey = n,
                    layui.data("layim", {
                        key: cache.mine.id,
                        value: l
                    }),
                    events.setSendHide(a, i.parent())
            }
        },
        setSendHide: function(i, a) {
            (a || events.setSend.box).hide().removeClass(anim)
        },
        face: function(i, a) {
            var n = ""
                , l = thisChat();
            for (var s in faces)
                n += '<li title="' + s + '"><img src="' + faces[s] + '"></li>';
            n = '<ul class="layui-clear layim-face-list">' + n + "</ul>",
                events.face.index = layer.tips(n, i, {
                    tips: 1,
                    time: 0,
                    fixed: !0,
                    ...
                    success: function(i) {
                        i.find(".layim-face-list>li").on("mousedown", function(i) {
                            ...
                            focusInsert(l.textarea[0], "face" + this.title + " "),
                                layer.close(events.face.index);
                        })
                    }
                }),
                e(document).off("mousedown", events.faceHide).on("mousedown", events.faceHide),
                e(window).off("resize", events.faceHide).on("resize", events.faceHide),
                stope(a)
        },
        faceHide: function() {
            layer.close(events.face.index)
            ...
            layui.upload.render({
                ...
                accept: a,
                    ...
                    0 == i.code ? (i.data = i.data || {},
                        "images" === a ? focusInsert(n.textarea[0], "img[" + (i.data.src || "") + "]") : "file" === a && focusInsert(n.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]"),
                        sendMessage()) : t.msg(i.msg || "上传失败")
                }
            })
        },
        media: function(i) {
        ...
        playAudio: function(i) {
            ...
            return i.data("error") ? t.msg("播放音频源异常") : e.play ? void (i.attr("status") ? laytpl() : (a || (e.src = i.data("src")),
                    e.play(),
                    i.attr("status", "pause"),
                    i.data("audio", e),
                    i.find("i").html("&#xe651;"),
        ...
        playVideo: function(i) {
            ...
            return e.play ? (layer.close(events.playVideo.index),
                void (events.playVideo.index = layer.open({
                    type: 1,
                    title: "播放视频",
                    area: ["460px", "300px"],
                    maxmin: !0,
        ...
        chatLog: function(i) {
            var a = thisChat();
            return cache.base.chatLog ? (layer.close(events.chatLog.index),
                events.chatLog.index = layer.open({
                    ...
                    content: cache.base.chatLog + "?id=" + a.data.id + "&type=" + a.data.type
                })) : t.msg("未开启更多聊天记录")
        },
        menuHistory: function(i, a) {
            var n = layui.data("layim")[cache.mine.id] || {}
                , l = i.parent()
                , s = i.data("type")
                , o = layimMain.find(".layim-list-history")
